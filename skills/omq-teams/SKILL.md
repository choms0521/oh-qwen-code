...
```bash
/oh-my-qwen:omq-teams N:claude "task description"
/oh-my-qwen:omq-teams N:codex "task description"
/oh-my-qwen:omq-teams N:gemini "task description"
```
...
- **Classic tmux session optional** for in-place pane splitting (`$TMUX` set). Inside cmux or a plain terminal, `omq team` falls back to a detached tmux session instead of splitting the current surface.
...
- If `$TMUX` is empty but `CMUX_SURFACE_ID` is set, report that the user is running inside **cmux**. Do **not** say tmux is missing or that they are "not inside tmux"; `omq team` will launch a **detached tmux session** for workers instead of splitting the cmux surface.
...
- For native Claude Code team agents/roles, direct them to **`/oh-my-qwen:team`** instead.
...
| `cmux surface detected`      | Running inside cmux without `$TMUX` | Use the normal `omq team ...` flow; OMC will launch a detached tmux session         |
| `Unsupported agent type`     | Requested agent is not claude/codex/gemini | Use `claude`, `codex`, or `gemini`; for native Claude Code agents use `/oh-my-qwen:team` |
...
| Invocation   | `TeamCreate` / `Task` / `SendMessage`     | `omq team [N:agent]` + `status` + `shutdown` + `api` |
