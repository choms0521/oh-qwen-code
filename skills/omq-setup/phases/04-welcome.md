...
- /oh-my-qwen:team 3:executor "fix all TypeScript errors"
- /oh-my-qwen:team 5:debugger "fix build errors in src/"
Teams use Claude Code native tools (TeamCreate/SendMessage/TaskCreate).

MCP SERVERS:
Run /oh-my-qwen:mcp-setup to add tools like web search, GitHub, etc.

HUD STATUSLINE:
The status bar now shows OMQ state. Restart Claude Code to see it.
...
```bash
gh api user/starred/Yeachan-Heo/oh-my-qwen &>/dev/null
```
...
**Question:** "If you're enjoying oh-my-qwen, would you like to support the project by starring it on GitHub?"
...
```bash
gh api -X PUT /user/starred/Yeachan-Heo/oh-my-qwen 2>/dev/null && echo "Thanks for starring!" || true
```
...
```bash
echo ""
echo "If you enjoy oh-my-qwen, consider starring the repo:"
echo "  https://github.com/Yeachan-Heo/oh-my-qwen"
echo ""
```
