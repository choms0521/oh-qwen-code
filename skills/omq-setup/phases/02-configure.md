...
This will:
1. Install the HUD wrapper script to `~/.claude/hud/omq-hud.mjs`
...
```bash
node -e "const p=require('path'),f=require('fs'),h=require('os').homedir(),d=process.env.CLAUDE_CONFIG_DIR||p.join(h,'.claude'),b=p.join(d,'plugins','cache','omc','oh-my-qwen');try{const v=f.readdirSync(b).filter(x=>/^\d/.test(x)).sort((a,c)=>a.localeCompare(c,void 0,{numeric:true}));if(v.length<=1){console.log('Cache is clean');process.exit()}v.slice(0,-1).forEach(x=>{f.rmSync(p.join(b,x),{recursive:true,force:true})});console.log('Cleared',v.length-1,'stale cache version(s)')}catch{console.log('No cache directory found (normal for new installs)')}"
```
...
const b=p.join(d,'plugins','cache','omc','oh-my-qwen');
...
// Try .omq-version.json second
if(v==='')try{const j=JSON.parse(f.readFileSync('.omq-version.json','utf-8'));v=j.version||''}catch{}
// Try CLAUDE.md header third
if(v==='')for(const c of['.claude/CLAUDE.md',p.join(d,'CLAUDE.md')]){try{const m=f.readFileSync(c,'utf-8').match(/^# oh-my-qwen.*?(v?\d+\.\d+\.\d+)/m);if(m){v=m[1].replace(/^v/,'');break}}catch{}}
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin oh-my-qwen"
  else
...
Store the preference in `~/.claude/.omq-config.json`:
...
The OMC CLI (`omc` command) provides standalone helper commands such as `omq hud`, `omq teleport`, and `omq team ...`.
...
**Question:** "Would you like to install the OMC CLI globally for standalone helper commands? (`omc`, `omq hud`, `omq teleport`)"
