...
```bash
grep -q "oh-my-qwen" ~/.claude/settings.json && echo "Plugin verified" || echo "Plugin NOT found - run: claude /install-plugin oh-my-qwen"
```
...
#### 3.3.3: Configure Team Defaults in omq-config
...
Store the team configuration in `~/.claude/.omq-config.json`:
...
Or by running `/oh-my-qwen:omq-setup --force` and choosing to enable teams.
