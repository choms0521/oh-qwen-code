---
name: omq-setup
description: Install or refresh oh-my-qwen for plugin, npm, and local-dev setups from the canonical setup flow
level: 2
---
...
- Marketplace/plugin install users should land here after `/plugin install oh-my-qwen`
...
```
OMC Setup - Configure oh-my-qwen

USAGE:
  /oh-my-qwen:omq-setup           Run initial setup wizard (or update if already configured)
  /oh-my-qwen:omq-setup --local   Configure local project (.claude/CLAUDE.md)
  /oh-my-qwen:omq-setup --global  Configure global settings (~/.claude/CLAUDE.md)
  /oh-my-qwen:omq-setup --force   Force full setup wizard even if already configured
  /oh-my-qwen:omq-setup --help    Show this help
...
EXAMPLES:
  /oh-my-qwen:omq-setup           # First time setup (or update CLAUDE.md if configured)
  /oh-my-qwen:omq-setup --local   # Update this project
  /oh-my-qwen:omq-setup --global  # Update all projects
  /oh-my-qwen:omq-setup --force   # Re-run full setup wizard

For more info: https://github.com/Yeachan-Heo/oh-my-qwen
```
...
CONFIG_FILE="$HOME/.claude/.omq-config.json"
...
Read the file at `${CLAUDE_PLUGIN_ROOT}/skills/omq-setup/phases/01-install-claude-md.md` and follow its instructions.
...
1. **Phase 1 - Install CLAUDE.md**: Read `${CLAUDE_PLUGIN_ROOT}/skills/omq-setup/phases/01-install-claude-md.md` and follow its instructions.

2. **Phase 2 - Environment Configuration**: Read `${CLAUDE_PLUGIN_ROOT}/skills/omq-setup/phases/02-configure.md` and follow its instructions. Phase 2 must delegate HUD/statusLine setup to the `hud` skill; do not generate or patch `statusLine` paths inline here.

3. **Phase 3 - Integration Setup**: Read `${CLAUDE_PLUGIN_ROOT}/skills/omq-setup/phases/03-integrations.md` and follow its instructions.

4. **Phase 4 - Completion**: Read `${CLAUDE_PLUGIN_ROOT}/skills/omq-setup/phases/04-welcome.md` and follow its instructions.
...
After installing oh-my-qwen updates (via npm or plugin update):

**Automatic**: Just run `/oh-my-qwen:omq-setup` - it will detect you've already configured and offer a quick "Update CLAUDE.md only" option that skips the full wizard.

**Manual options**:
- `/oh-my-qwen:omq-setup --local` to update project config only
- `/oh-my-qwen:omq-setup --global` to update global config only
- `/oh-my-qwen:omq-setup --force` to re-run the full wizard (reconfigure preferences)
